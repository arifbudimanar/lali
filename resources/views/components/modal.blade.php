...
<div x-data="{ show: @entangle($attributes->wire('model')) }" x-on:close.stop="show = false" x-on:keydown.escape.window="show = false" x-show="show"
    id="{{ $id }}" class="fixed inset-0 z-50 overflow-y-auto jetstream-modal sm:py-6 sm:px-6"
    style="display: none;">
    <div x-show="show" class="fixed inset-0 transition-all transform" x-on:click="show = false"
        ...
        <div class="absolute inset-0 bg-gray-300 opacity-70 dark:bg-gray-900"></div>
    </div>

    <div x-show="show"
        class="mb-6 bg-white dark:bg-gray-800 sm:rounded-lg overflow-hidden transform transition-all sm:w-full {{ $maxWidth }} sm:mx-auto"
